...
namespace A12
{
    public class AppAnalysis
    {
        ...
        public AppAnalysis(List<AppData> apps)
        {
            Apps = apps;
        }
        ...
        public static AppAnalysis AppAnalysisFactory(string appDataPath)
        {
            ...
            var appAnalysis = new AppAnalysis(Apps);
        ...
        {
            return Apps.Where(x => x.LastUpdate >= boundary).GroupBy(x => x.Categori)
                    .OrderByDescending(grp => grp.Count())
            .Select(grp => grp.Key).First();
        }
        public List<string> MostRatedCategories(double ratingBoundary, int n)
        {
            return Apps.Where(x => x.Rating > ratingBoundary).GroupBy(x => x.Categori)
        ...
        public Tuple<string, string> ExtremeMeanUpdateElapse(DateTime today)
        {
            ...
            first = Apps.GroupBy(a => a.Categori).OrderBy(a => a.Average(i => (today - i.LastUpdate).TotalDays))
                 .Select(i => i.Key).First();
            last = Apps.GroupBy(a => a.Categori).OrderBy(a => a.Average(i => (today - i.LastUpdate).TotalDays))
               .Select(i => i.Key).Last();
